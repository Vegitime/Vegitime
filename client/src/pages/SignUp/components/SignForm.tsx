...
export default function SignForm({formType}:{formType:string}) {
  const SIGNUP_LIST = formType === 'signIn' ? ['userId', 'userPwd'] : ['userId', 'userNickName', 'userPwd', 'userPwdCheck']
  return (
    <Form>
      <fieldset>
        <legend className="sr-only">{formType === 'signIn' ? '로그인 폼' : '회원가입 폼'}</legend>   
        {SIGNUP_LIST.map((id) => <SignInputContainer id={id as ('userId' | 'userNickName' | 'userPwd' | 'userPwdCheck')} formType={formType} key={formType+id}/>)}        
        {formType === 'signIn' 
          ? <FormErrorMessage formType="signin">일치하는 회원 정보가 없어요. 다시 시도해주세요.</FormErrorMessage>
          : null}
        <FormButton isActive={true} type="submit">{formType === 'signIn' ? '로그인' : '회원가입'}</FormButton>
      </fieldset>
    </Form>
  )
}
